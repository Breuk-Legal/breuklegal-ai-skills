...
Cuando una factura cruza el umbral de prejuridico a judicial (senalado por `cobro-ingesta-clasificacion`), no generes, redactes ni envies ningun documento. Genera unicamente una alerta dirigida al usuario con este contenido: la factura y deudor en cuestion, el umbral que se cumplio, y los datos de contacto del `contacto_escalamiento` configurado (agenda, correo, WhatsApp) para que el usuario lo contacte directamente. Nada mas.
...
- Nunca olvides registrar en el registro de comunicaciones lo que efectivamente salio — sin eso, `cobro-ingesta-clasificacion` no puede clasificar bien la proxima vez.
