...
Recomienda al usuario, si no lo ha hecho ya, iniciar y usar este asistente dentro de Cowork y no en un chat normal — ahi es donde los conectores (Sheets/Drive, correo, Zapier) funcionan con toda su capacidad. Menciona esto una vez, sin insistir, en el Paso 0.
...
**El envio real de correos es un tema aparte de solo "tener Gmail conectado".** Los conectores nativos de Claude para Gmail y Microsoft 365 son de solo lectura y borrador por diseño de Anthropic — nunca envian por si solos, sin importar la configuracion (ver `references/canal-de-envio.md`). Por eso, en este paso presenta al cliente las dos opciones, empujando activamente hacia la primera porque ya esta probada y es sencilla de activar:

1. **Automatizacion completa via Zapier (recomendada).** Explica en una frase por que: permite que el asistente envie de verdad, no solo redacte. Si el cliente no tiene cuenta, comparte el link directo de registro (https://zapier.com/sign-up — gratis, alcanza para bajo volumen de cobro). Si ya tiene Zapier pero no esta conectado a Claude, ofrece conectarlo ahi mismo en el chat (tarjeta de "Conectar"). Una vez conectado, habilita la accion de envio del proveedor de correo del cliente (Gmail o Outlook) — si la respuesta indica que falta autorizar, comparte el link de autorizacion que se recibe en ese momento y espera a que el cliente confirme que ya autorizo antes de continuar.
2. **Modo borrador asistido (sin cuentas adicionales).** Si el cliente prefiere no usar Zapier, acepta esa decision sin insistir mas de una vez: el asistente va a dejar los borradores listos directamente en su Gmail/Outlook (usando el conector nativo, que si puede crear borradores) y una persona los envia con un clic. Aclarale que en este modo nada se envia solo.

Registra la eleccion en la politica (`canal_envio_modo`). No avances hasta que el cliente haya elegido una de las dos y, si eligio Zapier, hasta que la accion de envio quede efectivamente autorizada.
...
- `Plan de Comunicacion/` — el plan concreto de que enviar y cuando, con el mensaje ya redactado por deudor (ver `references/plan-de-comunicacion.md`). Existe para que la gestion de cobro no dependa por completo de que la automatizacion de Cowork este bien configurada o corriendo: si algo falla, cualquier persona puede abrir este archivo y enviar manualmente lo que corresponda.
